...
def load_chroma_config(config_path:str = get_abs_path(r'config/chroma.yml'), encoding:str = 'utf-8' ):
    with open(config_path,'r',encoding=encoding) as f:
        return yaml.load(f, Loader=yaml.FullLoader) # 全量加载

def load_prompts_config(config_path:str = get_abs_path(r'config/prompts.yml'), encoding:str = 'utf-8' ):
    with open(config_path,'r',encoding=encoding) as f:
        return yaml.load(f, Loader=yaml.FullLoader) # 全量加载
